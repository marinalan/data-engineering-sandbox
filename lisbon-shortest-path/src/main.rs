...
fn main() {
    ...
    let abril_bridge = graph.add_node("25 de Abril Bridge");
    ...
    graph.extend_with_edges([
        ...
        (abril_bridge, belem_tower, 1), // The distance from Commerce Square to Lisbon Cathedral is 1 km
        (abril_bridge, lx_factory, 2), // The distance from Commerce Square to Lisbon Cathedral is 1 km
    ]);
    ...
    let node_map = dijkstra(&graph, abril_bridge, Some(monastery), |e| *e.weight());

    if let Some(distance) = node_map.get(&monastery) {
        println!(
            "The shortest distance from 25 de Abril Bridge to Monastery is {} km",
            distance
        );
    } else {
        println!("No route found from 25 de Abril Bridge to Monastery.");
    }
}
